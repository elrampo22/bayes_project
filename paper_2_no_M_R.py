...
def sample_data_from_function(n = 10,c=0.05):
...
def sample_true_function(x_start=0,x_end=0.4,n=100):
    #generate x values
...
x,y, sigmas = sample_data_from_function()
print(y)
x_true, y_true = sample_true_function()
...
fig = corner.corner(
    ...
    fill_contours=False,
    levels=1-np.exp(-0.5*np.array([0.5, 1,1.5,2])**2),  # Confidence levels
    color="black",
    bins=50
)
...
plt.show()
if prior_type == 'C*':
    plt.savefig(f"corner_Cprime_{r}_{M_Max}_afix_{prior_defaults['a_fix']}.pdf",dpi=400)
elif prior_type == 'A*':
    plt.savefig(f"corner_Aprime_{r}_{M_Max}_afix_{prior_defaults['a_fix']}.pdf",dpi=400)
plt.clf()
...
#Estimated model
a_res_mean = np.mean(a_res_samples, axis=0)
x_estim,y_estim = sample_estimated_function(a_res_mean)
plt.plot(x_estim, y_estim, label=f'$k = {r}, \\; M_{{max}} = {M_Max}$', color='red', linestyle='--')
...
sampling_x = np.linspace(0,0.4,100,endpoint=True)
processed_rows = []
...
plt.show()
if prior_type == 'C*':
    plt.savefig(f"fitted_function_Cprime_{r}_{M_Max}_afix_{prior_defaults['a_fix']}.pdf",dpi=400)
elif prior_type == 'A*':
    plt.savefig(f"fitted_function_Aprime_{r}_{M_Max}_afix_{prior_defaults['a_fix']}.pdf",dpi=400)
plt.clf()
...
_,mu = sample_estimated_function(a_res_mean,x)
chi_squared_value = chi_squared(y,sigmas,mu)
print('Chi-squared value:',np.round(chi_squared_value,3))
print('Chi-squared/dof:',np.round(chi_squared_value/(10-(r+1)),3))
...
print(np.mean(all_likelihoods))
print(a_res_samples.shape,all_likelihoods.shape)
